...
- **Seven init settings are native only.** `memoryPoolSize`, `threadAttributes`, and `logFile` are skipped with one warning on HTML5. `logFlags` and `logLevel` reach FMOD's logger, which the web package does not export (see the logging bullet above). `streamingScheduleDelay` and `encryptionKey` are accepted and have no effect there. The web build allocates from the wasm heap and runs on the browser's audio thread. `output` accepts only `WEBAUDIO`, `AUDIOWORKLET`, `NOSOUND`, and `NOSOUND_NRT` there. Any other value fails init with `FMOD_ERR_UNSUPPORTED`.
