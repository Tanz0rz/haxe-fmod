...
- **Initialization is asynchronous.** The wasm module and the default banks load in the background. `FmodManager.IsInitialized()` reports true once both are usable. Games gate their first scene on it. Native targets initialize synchronously, so the same polling code works everywhere.
