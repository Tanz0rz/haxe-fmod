...
- **Readback and profiling queries are native only.** The web glue cannot hand these payloads to JavaScript, so each is a compile error in a js build and returns its empty value with the opt-out: `getFadePoints` and `getMixMatrix` on `Channel` and `ChannelGroup`, `DspConnection.getMixMatrix`, `CoreSystem.getDefaultMixMatrix` and `getDspInfoByType`, `Dsp.getParameterInfo`, `getLoudnessMeterInfo`, `getLoudnessMeterWeighting`, `getPluginInfo`, and `addInputPreallocated`, `Sound.lock` and `unlock`, `CoreSystem.setDiskBusy` and `getDiskBusy`, `Bus.getPortIndex` and `setPortIndex`, and the `getCpuUsage` and `getMemoryUsage` queries on `Bus`, `EventInstance`, and `StudioSystem`.
