...
/**
 * CI test scenario for the Kha attachment components. Logs one
 * "PAN_TEST:" line per check. CI gates on "PAN_TEST: COMPLETE" with no
 * "pass=false".
 *
 * The flow copies the flixel EmitterPanTestState. An emitter tracks a
 * body. A dispose call detaches the emitter and releases the instance. A
 * listener drives the listener position. Distance culling and the
 * attached one-shot helper run here as well. The scenario meters real
 * spatialization on the Spatial event's channel group.
 *
 * Kha bodies carry no velocity. Each check that needs a known velocity
 * ticks the component with a fixed elapsed time, then moves the body by a
 * known distance. The camera checks use the listener's sampler mode
 * with a fake camera, since Kha has no camera of its own.
 *
 * Select via HAXEFMOD_TEST_STATE=pan-test (native) or ?test=pan-test (browser).
 */
class PanTestScenario implements TestScenario {
    ...
    function sprite(x:Float, y:Float):Sprite {
    ...
    public function create(testHost:TestHost):Void {
        ...
        // Warm the event description cache. Its lookup allocates a single
        // persistent deduped handle. The next step captures the leak
        // baseline. After it, the emitter's instance is the only
        // allocation, and dispose() releases it.
        ...
        var emitter = FmodKhaEmitter.play(FmodEvents.MusicMainLevel, target);
        ...
        target.x = 300;
        target.y = 220;
        var listener = new FmodKhaListener(target);
        ...
        var loader = new FmodKhaBankLoader([], () -> loaderFired = true);
        loader.tick(0);
        ...
        var trigger = new FmodKhaParameterTrigger(target, 0, 0, 200, 200, "Nope", 1, 0);
    ...
    var _listenerSprite:Sprite;
    var _cullSprite:Sprite;
    var _cullEmitter:FmodKhaEmitter;
    var _targetListener:FmodKhaListener;
    var _phase:String = "";
    ...
    /**
     * Runs the culling flow against a looping event that carries an
     * explicit cull distance. The example bank has no authored 3D
     * distances. The flow culls when far and restarts when near. It also
     * restarts when culling is disabled mid-cull, and it leaves one-shots
     * alone entirely.
     */
    function startCullPhases():Void {
        ...
        _cullEmitter = FmodKhaEmitter.play(FmodEvents.MusicMainLevel, _cullSprite);
        ...
        switch (_phase) {
            ...
            case "loop_restarts_when_disabled":
                ...
                if (playing || timedOut) {
                    ...
                    _cullSprite.x = FAR;
                    _cullEmitter = FmodKhaEmitter.play(FmodEvents.SFXJump, _cullSprite);
                    _cullEmitter.stopEventsOutsideMaxDistance = true;
                    _cullEmitter.cullMaxDistance = 500;
                    _cullEmitter.cullCheckInterval = 1;
    ...
    function finishCullPhases():Void {
        ...
        FmodKhaUtilities.PlayOneShotAttached(FmodEvents.SFXJump, _listenerSprite);
    ...
    var _spatialSprite:Sprite;
    var _spatialMoving:Bool = false;
    var _spatialEmitter:FmodKhaEmitter;
     ...
     * The listener tracks the listener object's center at 308, 228.
     * Metering runs on the instance's channel group. The metering DSP sits
    ...
    function startSpatialEmitter():Void {
        ...
        _spatialEmitter = FmodKhaEmitter.play(FmodEvents.SFXSpatial, _spatialSprite);
    ...
    /**
     * Sampler mode: velocity derived from a camera center's per-frame
     * movement, with jumps beyond teleportDistance treated as cuts.
     * Driven manually with a fixed elapsed so the derived velocity is
     * deterministic. Kha has no camera, so a pair of values stands in.
     */
    function runCameraListenerChecks():Void {
        ...
        var cameraX = 100.0;
        var cameraY = 80.0;
        var cameraListener = new FmodKhaListener();
        cameraListener.setSampler(() -> cameraX + 160, () -> cameraY + 120);
        ...
        if (attributes != null) {
            check("camera_listener_position_is_center",
                approx(attributes.position.x, cameraX + 160) && approx(attributes.position.y, cameraY + 120),
    ...
    /**
     * A real zone crossing feeds a real event parameter through the
     * trigger's instance variant. A second contract holds here - manual
    ...
    function runParameterTriggerChecks():Void {
        ...
        var trigger = new FmodKhaParameterTrigger(
            target, 250, 150, 200, 200, param.name, param.maximum, param.minimum, inst);
        trigger.tick(0);
        ...
        var globalTrigger = new FmodKhaParameterTrigger(
            target, 250, 150, 200, 200, "Intensity", 1, 0);
...
/** A body for the components to follow, 16 by 16 like the flixel sprite. */
class Sprite {
    public var x:Float;
    public var y:Float;
