...
const HAXEFMOD_EXAMPLES = {
 ...
 "loading-and-playing-sounds-in-the-core-api": {
  "4.1.1 Non-blocking Sound Creation": {
   "code": "var sound = Sound.create(\"../media/wave.mp3\", false, false, ChannelMode.CREATESTREAM | ChannelMode.NONBLOCKING); // Creates a handle to a stream then commands the FMOD Async loader to open the stream in the background.\nif (sound.isNull()) trace(StudioSystem.lastResult());",
   "member": null,
   "notes": [],
  ...
  "4.3.2 Creating a Sound from PCM data": {
   ...
   "member": null,
   "notes": [],
   "type": "haxefmod.core.ChannelMode, haxefmod.core.Sound, haxefmod.studio.Types",
   "verdict": "bound"
  },
 ...
 "studio-api-system": {
  ...
  "FMOD_STUDIO_LOAD_MEMORY_ALIGNMENT": {
   ...
   "member": "STUDIO_LOAD_MEMORY_ALIGNMENT",
   "notes": [],
   "type": null,
   "verdict": "bound"
  },
...
// It does the same for every other code block from HAXEFMOD_EXAMPLES
// (examples-data.js, generated by ci/haxe-catalog.py), keyed by
// extension/keys.js.
//
// The site's own selector only knows its four language classes. The
...
(function () {
    ...
    // offers and rewrites the key when it finds ours. The Haxe choice
    ...
    var GUIDES = "https://www.tanz0rz.com/haxe-fmod/";
    ...
    // An example with no Haxe equivalent is a comment on its own.
    ...
    // tab per language. It reads like any tabbed unit on the site.
    ...
    //
    // A lone block on a page with a language selector of the site's own
    // is already governed by it. The site shows and hides every
    // language-classed block on the page. The Haxe translation just
    ...
    // (the guides and platform pages) gets a strip per lone unit. There
    // is no other place to pick Haxe from.
    function injectAll() {
    ...
    // The browser's scroll anchoring then shifts the page so some
    // other element stays put. The tab that was clicked is what the eye
    // is on. Its viewport position is taken before any handler runs
