...
`ci/example-line-counts.py` counts the lines of the site's C# snippet (the C++ one where a page has no C#) against the lines the Haxe tab shows for the same unit. A difference is reviewed once and recorded with its reason in `test/line-count-waivers.md`. The script reports a unit again when either count moves and asks for the row to go when the counts match. `--show <page> "<key>"` prints both snippets side by side, `--html <file>` writes a page with every waived unit side by side for review by eye, and `--fetch <dir>` downloads the page fragments from the docs content origin and rebuilds the catalog from them.
...
On a machine where the site does not render, `--check --from <dir>` reads saved content fragments instead (one `<page>.html` per file from the docs content origin, `https://d1s9dnlmdewoh1.cloudfront.net/2.03/api/<page>.html`), which carry the same markup the site injects.
