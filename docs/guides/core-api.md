...
`Sound.create(path, ?loop, ?openOnly, ?mode, ?initialSubsound, ?exinfo)` loads a file from disk. `mode` takes any combination of `ChannelMode` flags on top of what `loop` and `openOnly` set, so a sound can be 3D, streamed, decoded up front, or kept compressed from the moment it loads. `NONBLOCKING` returns at once with the load running on FMOD's thread, and `getOpenState()` reports `LOADING` until it is `READY` or `ERROR`. `initialSubsound` picks the subsound an FSB stream starts on, and `exinfo` is an `FmodCreateSoundExInfo` for the rest of `FMOD_CREATESOUNDEXINFO` (raw PCM format, file offset and length, buffer sizes, encryption key, and so on). `fromMemory` takes the same struct as its last argument.
...
A `ChannelGroup` is a mixing bus for raw channels. `ChannelGroup.master()` is the final mix everything passes through. `create(name)` makes a custom group, and `Channel.setChannelGroup` routes a channel into it. Groups nest through `addGroup(child, ?propagateDspClock)`, or `addGroupConnection` when the `DspConnection` between the two is wanted, and each carries volume, pitch, mute, pause, pan, lowpass, a DSP chain, 3D attributes, and fade points scheduled against the mixer clock. `isPlaying()`, `getDelay()`, `getLowPassGain()`, and `get3DOcclusion()` read a group back like a channel, and `ChannelGroup.setCallback` delivers `Occlusion` events for 3D groups when geometry is in use.
...
`getParameterInfo(index)` returns the parameter's `FmodDspParameterDesc`, its name, type, and the union member for that type (`floatDesc` with range, default, and mapping, `intDesc`, `boolDesc`, or `dataDesc`), which is what a settings screen needs to build a slider for an effect it did not author (unsupported in HTML5). It is a compile error there unless the project opts in, and then returns `null` with `FMOD_ERR_UNSUPPORTED` in `StudioSystem.lastResult()`. `getDataParameterIndex(dataType)` finds the data parameter carrying a given `FMOD_DSP_PARAMETER_DATA_TYPE`. The small data structs have typed accessors that pack the FMOD layout in the shim: `setParameterSidechain(index, props)` and `getParameterSidechain(index)` (`DspCompressor.USESIDECHAIN`), `setParameterFiniteLength` and `getParameterFiniteLength`, `setParameterAttenuationRange` and `getParameterAttenuationRange` on the pan units, `getParameterDynamicResponse(index)`, and `setLoudnessMeterWeighting(weighting)` with `getLoudnessMeterWeighting()` (unsupported in HTML5) on a loudness meter. `setChannelFormat`, `getChannelFormat`, and `getOutputChannelFormat` fix and read the unit's channel mask, channel count, and speaker mode.
...
Three effects double as analyzers. A `DspType.FFT` unit attached where you want to listen exposes `getFftSpectrum(maxBins)` for its first channel and `getFftSpectrumInfo(maxBins)` for every channel with the bin and channel counts. Any unit reports `getMetering()` (an `FmodDspMeteringInfo` with `peakLevel` and `rmsLevel` per output channel, `numChannels`, and `numSamples`) once `setMeteringEnabled` is on, and `getMetering(true)` or `getInputMetering()` reads the input side. A `DspType.LOUDNESS_METER` unit reports `getLoudnessMeterInfo()`, its momentary, short term, and integrated loudness in LUFS with the histogram and true peak (unsupported in HTML5). Every unit also has `getInfo()` for its name, version, and channel count, `getParameterData(index)` for the raw bytes behind any data parameter, and `getOverallGain()` for the gain it reports to the virtual voice system. A pan or object panner unit created by hand takes its position through `setParameter3DAttributesMulti(index, absolute, relative, ?weights)`, and a plugin parameter of the single 3D attributes type through `setParameter3DAttributes(index, absolute, ?relative)`, both packed by the shim.
...
`Dsp.play` plays a generator unit such as `DspType.OSCILLATOR` as a sound source. `addInput`, `disconnectFrom(input, ?connection)`, and the `DspConnection` handle build custom mixer topologies, including send and sidechain routings through `DspConnection.TYPE_SEND` and `TYPE_SIDECHAIN` with a per-connection mix level. Any graph change invalidates every connection handle, so query them again through `getInputConnection` afterwards. `DspConnection.setMixMatrix(matrix, outChannels, inChannels, ?inChannelHop)` routes an input's channels to the output's with explicit gains, and `getMixMatrix()` reads the matrix and the channel counts back (unsupported in HTML5). It is a compile error there unless the project opts in, and then returns `null` with `FMOD_ERR_UNSUPPORTED` in `StudioSystem.lastResult()`.
...
`getMemoryStats(?blocking)` reports the bytes FMOD currently has allocated and the most it has ever had. `blocking` defaults to true, which makes FMOD flush pending commands first so the numbers are exact, and false reads the counters as they stand. `getFileUsage()` reports the bytes FMOD has read from disk since init, split into sample loads, streams, and everything else. Both work on every target and return `null` on failure.

### Command capture and replay

`StudioSystem.startCommandCapture(path, ?flags)` records every Studio API call to a file that FMOD's tools can analyze, and `stopCommandCapture()` ends it. The `FmodCommandCaptureFlags` are `FILEFLUSH` to write each command to disk as it happens and `SKIP_INITIAL_STATE` to leave out the commands that recreate the current state. `loadCommandReplay(path, ?flags)` loads a capture as a `CommandReplay` handle that plays the recorded calls back through the live system with `start`, `stop`, `setPaused`, and `seekToTime(seconds)`. Its `FmodCommandReplayFlags` are `FAST_FORWARD`, `SKIP_CLEANUP`, and `SKIP_BANK_LOAD`. The handle also exposes the capture itself. `getCommandCount()` reports how many commands it holds, `getCommandInfo(index)` returns an `FmodCommandInfo` with the command name, frame number, frame time, and the instance and output it applied to, and `getCommandString(index)` formats one command the way FMOD's tools print it. `getCommandAtTime(seconds)` finds the command playing at a point in the capture, `seekToCommand(index)` moves playback there, `getPlaybackState()` reports the replay's state, and `setBankPath(directory)` tells the replay where to load banks from when the captured paths no longer apply.
