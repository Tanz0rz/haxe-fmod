...
`EventInstance.assignProgrammerSound(key)` names the audio a programmer instrument should play. Assign it before `start()`, and the native side resolves it on FMOD's thread when the instrument triggers. The key is either an audio table entry or a file path. Keys must be under 512 UTF-8 bytes. The sound is created with `FMOD_NONBLOCKING`, so the decode runs off the Studio thread and FMOD waits for it before the instrument plays. Programmer sounds fail inside FMOD's JavaScript runtime, documented in [Limitations](../limitations.md#html5), so every `assignProgrammerSound` variant is a compile error in a js build. With `-D haxefmod_html5_allow_unsupported` the calls compile and return `FMOD_ERR_UNSUPPORTED` in the browser. `StudioSystem.getSoundInfo(key)` reports what FMOD would load for an audio table key, the file name or path and the subsound index inside it, and returns `null` for a key that is in no loaded audio table.
