 ...
 * project.xml haxedefs. Two hxml files carry the exact CI invocations:
 * tests/build-defines.hxml and tests/build-debug-defaults.hxml. The
 * second one compiles with -debug and no haxefmod defines, which covers
 * the debug build's liveUpdate default. The first one passes these
 * defines:
 *
