...
/**
 * Tests the version-parsing logic in PostBuild.
 *
 * Each case calls PostBuild.hexToVersion or PostBuild.parseFmodVersion
 * directly. A change to either algorithm shows up here as a failure.
 */
class TestVersionParsing {
	...
	static function testHexToVersion_2_03_12() {
		assert("0x00020312 -> 2.03.12", PostBuild.hexToVersion("0x00020312") == "2.03.12");
	}

	static function testHexToVersion_2_02_00() {
		assert("0x00020200 -> 2.02.00", PostBuild.hexToVersion("0x00020200") == "2.02.00");
	}

	static function testHexToVersion_1_10_20() {
		assert("0x00011020 -> 1.10.20", PostBuild.hexToVersion("0x00011020") == "1.10.20");
	}

	static function testHexToVersion_invalidReturnsInput() {
		assert("invalid hex returns input", PostBuild.hexToVersion("garbage") == "garbage");
	}
	...
	static function testParseFmodVersion_standardHeader() {
		...
		assert("parses standard header", PostBuild.parseFmodVersion(path) == "0x00020312");
	}
	...
	static function testParseFmodVersion_extraWhitespace() {
		...
		assert("parses with extra whitespace", PostBuild.parseFmodVersion(path) == "0x00020312");
	}
	...
	static function testParseFmodVersion_noVersionLine() {
		...
		assert("returns null when no version line", PostBuild.parseFmodVersion(path) == null);
	}
	...
	static function testParseFmodVersion_commentedOut() {
		...
		// The commented-out line contains both keywords, so the parser
